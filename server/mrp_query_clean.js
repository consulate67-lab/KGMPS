...
const getMrpQuery = (prodLocs, rawLocs) => {
  ...
  return `
    ...
    From (siparis_kay sk join siparis_har sh on (sk.SipNo=sh.SipNo) and ((RTRIM(sh.Durum)='')or(sh.durum is null)))
    join si_gchar si on (si.skod=sh.skod) and (RTRIM(si.Modul)='i') and (sk.SipNo=si.FisNo) and (sh.SipHarinx=si.FisHarInx) and (RTRIM(si.location)=RTRIM(sk.Location))
    left outer join Urt_Plan_Model upm on (upm.SipNo=sk.SipNo) and (upm.SipHarinx=sh.Sipharinx) and (upm.Activid=0)
    left outer join Urt_Plan_ozdur upo on (upo.SipNo=sk.SipNo) and (upo.SipHarinx=sh.Sipharinx) and (upo.urkod=si.rkod) and (upo.ubedkod=si.Bedkod) and (upo.Activid=0) and (RTRIM(upo.HEvent)='*')
    join model_PD pd on (RTRIM(pd.ModelKod) = RTRIM(isnull(upm.ModelKod,''))) and (RTRIM(upo.proses)=RTRIM(pd.proses)) and (upo.parcainx=pd.parcainx)
    Left Outer join StokKart st1 on (st1.SKod=sh.SKod)
    Left Outer Join StokKart st2 on (st2.SKod=pd.SKod)
    Where (RTRIM(sk.SipTip)='S') AND (RTRIM(sk.SipTur) = 'N') AND ((RTRIM(sk.Durum)='')or(sk.Durum is Null)) AND (RTRIM(sk.Location) IN (${pLocs}))
    ...
    ) pd on pd.sipno=sk.sipno and pd.sipharinx=sh.sipharinx and RTRIM(pd.modelkod)=RTRIM(upm.modelkod) and pd.activid=upm.activid and pd.URKod=upo.URKod and pd.UBedKod=upo.UBedkod and RTRIM(pd.Proses)=RTRIM(upo.proses) and pd.Parcainx=upo.Parcainx and RTRIM(pd.SKod)=RTRIM(upo.HSKod) and pd.HRKod=upo.HRKod and pd.HBedKod=upo.HBedKod
    Left Outer join StokKart st1 on (st1.SKod=sh.SKod)
    Left Outer Join StokKart st2 on (st2.SKod=pd.SKod)
    Where (RTRIM(sk.SipTip)='S') AND (RTRIM(sk.SipTur) = 'N') AND ((RTRIM(sk.Durum)='')or(sk.Durum is Null)) AND (RTRIM(sk.Location) IN (${pLocs}))
    Group by si.skod, si.Rkod, si.BedKod, si.FisNo, si.FisHarInx, isnull(upm.ModelKod,''), pd.Proses, pd.Parcainx, pd.SKod, pd.Miktar, pd.Birim, pd.Tip, pd.Location, pd.Tanim, pd.Resim, pd.HMikMod, pd.PrintOp, st1.bedkod, st2.bedkod;
    ...
    update pd set r1 = (select rkod from Model_PDS2R s2r Where (RTRIM(s2r.ModelKod)=RTRIM(pd.ModelKod)) and(RTRIM(s2r.Proses)=RTRIM(pd.Proses))and(s2r.Parcainx=pd.Parcainx) and (RTRIM(s2r.skod)=RTRIM(xSKod))) from @Model_Pd_si pd where (r1 is null) and not exists(select top 1 1 from s_gchar sgrr where RTRIM(sgrr.skod)=RTRIM(xSKod) and RTRIM(sgrr.modul)='X' and sgrr.FisNo=0 and sgrr.FisHarInx=0 and RTRIM(sgrr.Location)='');
    update pd set r1= (select top 1 isnull(RKod,0) from s_gchar sghh where (RTRIM(sghh.skod)=RTRIM(pd.SKod))and(sghh.BedKod=0)and(RTRIM(sghh.Modul)='X') and FisNo=0 and FisHarInx=0 and RTRIM(Location)='') from @Model_Pd_si pd where (r1 is null) and not exists(select top 1 1 from s_gchar sgrr where RTRIM(sgrr.skod)=RTRIM(xSKod) and RTRIM(sgrr.modul)='X' and sgrr.FisNo=0 and sgrr.FisHarInx=0 and RTRIM(sgrr.Location)='');
    
    update pd set b1= (select xkod from Model_PDS2X s2x Where (RTRIM(s2x.ModelKod)=RTRIM(pd.ModelKod)) and(RTRIM(s2x.Proses)=RTRIM(pd.Proses))and(s2x.Parcainx=pd.Parcainx) and (RTRIM(s2x.skod)=RTRIM(xSKod))) from  @Model_Pd_si pd Where (b1 is null) and (isnull(uBedGrp,'')='');
    update pd set b1= (select min(bedinx) From Dbo.P_Beden_D dd Where RTRIM(dd.BedKod)=RTRIM(hBedGrp)) from  @Model_Pd_si pd Where (b1 is null) and (isnull(uBedGrp,'')='');
    ...
    left outer join @urtln xxxx on RTRIM(xxxx.skod)=RTRIM(pd.xSKOD) and xxxx.rkod=pd.xrkod and xxxx.bkod=pd.xbkod and xxxx.FisNo=pd.xFisNo and xxxx.Finx=pd.xFinx and RTRIM(xxxx.MKod)=RTRIM(pd.ModelKod)
    left outer join Urt_Plan_ozdur upo on (upo.SipNo=sk.SipNo) and (upo.SipHarinx=sh.Sipharinx) and (upo.urkod=si.rkod) and (upo.ubedkod=si.Bedkod) and (RTRIM(upo.proses)=RTRIM(pd.proses)) and (RTRIM(isnull(upo.hskod,''))=RTRIM(isnull(pd.skod,''))) and (upo.parcainx=pd.parcainx) and (upo.Activid=0)
    Left Outer join StokKart st1 on (st1.SKod=sh.SKod)
    Left Outer Join Model_M MoMa On (RTRIM(MoMa.ModelKod)=RTRIM(pd.ModelKod))
    ...
    Where (RTRIM(sk.SipTip)='S') and (isnull(upo.HEvent,'')<>'-') and (xxxx.FisNo is not null) AND (RTRIM(sk.SipTur) = 'N') AND ((RTRIM(sk.Durum)='')or(sk.Durum is Null)) AND (RTRIM(sk.Location) IN (${pLocs}))
