const getMrpQuery = (prodLocs, rawLocs) => {
  const formatLocs = (locString) => {
    if (!locString) return "''";
    // Sona gelebilecek fazladan virgülleri temizle ve her kodu tek tırnak içine al
  ...
  return `
    ...
    LEFT JOIN Dbo.P_RNK_Tip rn ON rn.Renk_kod = pd.HRKod -- Eğer model_PD'de HRKod varsa
    GROUP BY pd.SKod, st_ham.Tanim, rn.Tanim, pd.Birim
